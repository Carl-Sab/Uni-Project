...
completed attempts of the same course exist is never exercised by
`verify_data.py` against real data. These tests construct that case
synthetically.
"""

from decimal import Decimal

from app.scripts.verify_data import _gpa, _total_credits_earned, dedupe_best_attempts

# Row shape matches the query in _best_attempt_per_course:
...
def test_credit_earned_once_despite_two_completed_attempts():
    ...
    assert _total_credits_earned(best) == 3  # not 6
...
def test_w_and_p_excluded_from_gpa_but_p_earns_credit():
    ...
    assert _total_credits_earned(best) == 6  # HUMN 210 (P) + MATH 101, not SOCI 240 (W)
